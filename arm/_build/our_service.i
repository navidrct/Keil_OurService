...
#line 39 "..\\our_service.c"
#line 40 "..\\our_service.c"
...
void our_service_init(ble_os_t * p_our_service)
{
    uint32_t   err_code; 
		...
		p_our_service->conn_handle = 0xFFFF;
    
    ble_uuid_t        service_uuid;
		ble_uuid128_t     base_uuid = {0x23, 0xD1, 0x13, 0xEF, 0x5F, 0x78, 0x23, 0x15, 0xDE, 0xEF, 0x12, 0x12, 0x00, 0x00, 0x00, 0x00};
		service_uuid.uuid = 0xABCD;
		err_code = sd_ble_uuid_vs_add(&base_uuid, &service_uuid.type);
		do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 59, (uint8_t*) "..\\our_service.c"); } while (0); } } while (0);
		
    
		err_code = sd_ble_gatts_service_add(0x01,
                                    &service_uuid,
                                    &p_our_service->service_handle);
		do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 65, (uint8_t*) "..\\our_service.c"); } while (0); } } while (0);
...
void our_char_add(ble_os_t * p_our_service)
{
	...
	char_uuid2.uuid      = 0xBEED;
	
	err_code = sd_ble_uuid_vs_add(&base_uuid, &char_uuid.type);
	err_code = sd_ble_uuid_vs_add(&base_uuid, &char_uuid2.type);
	do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 89, (uint8_t*) "..\\our_service.c"); } while (0); } } while (0);
	...
	do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 138, (uint8_t*) "..\\our_service.c"); } while (0); } } while (0);
