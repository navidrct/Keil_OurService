                                       ...
                                       pstorage_handle_t * p_block_id)
{
    ...
    do { if (!m_module_initialized) { return ((0x0) + 8); } } while(0);
    if ((p_lbse_id) == 0) { return ((0x0) + 14); };
    if ((p_block_id) == 0) { return ((0x0) + 14); };
    if ((((p_lbse_id)->module_id) >= 1) || (m_app_table[(p_lbse_id)->module_id]. cb == 0)) { return ((0x0) + 7); };

    temp_id           = (*p_lbse_id);
    temp_id.block_id += (block_num * (m_app_table[(p_lbse_id)->module_id]. block_size));
